    ...
    <div className="min-h-screen bg-background pb-24 lg:pb-0">
      ...
      <main className="mx-auto max-w-2xl px-4 py-12">
        <nav className="mb-8 flex items-center gap-1.5 text-xs text-muted-foreground">
          ...
          <span className="text-foreground">{t.footer.help.sellerReg}</span>
        </nav>

        <div className="mb-8 flex flex-col items-center rounded-2xl border border-border bg-card p-8 text-center">
          <Store className="mb-4 h-16 w-16 text-accent" />
          <h1 className="mb-2 font-serif text-2xl font-bold text-foreground">
            {t.footer.help.sellerReg}
          </h1>
          <p className="mb-6 text-muted-foreground">
            انضم كبائع على منصة xoperfumes ووصل بعروضك إلى آلاف العملاء في فلسطين والأردن.
          </p>
          <Button asChild>
            <Link href="/contact">تواصل معنا للتسجيل</Link>
          </Button>
        </div>
      </main>
